- **Introduction**
    - It is now obvious that contracts signed between entities or individuals, wet contracts, do not convert into smart contracts easily. In fact, with the strict rules governing how smart contracts function, it's not even clear this is feasible. Send Protocol builds a ramp for contracts to enter DeFi, and it's not what you might assume.
    - The internet gave rise to cloud computing. Cloud was Google's answer to the problem of indexing and search a constantly growing catalog of web pages. This required vast concentrated computing resources and an application stack that makes it possible. This stack turned out to be great at hosting other types of applications, and especially multi-user applications that are provided in the Software as a Service (SaaS) model. These globally shared applications are useful, because they provide a single source of truth and business rules that shows current information to its users and allows them to update this information according to pre-agreed terms. 
    - Despite their ubiquity and utility, cloud applications nevertheless have a fatal flaw, which is that they need to be a single source of truth. This means that in order to benefit from them every user must have an account on the system and have access to some of its data.
    - With blockchains acting as a state machine, new solutions are now possible, where records are distributed among many systems with the blockchain acting as a layer that can help to synchronise these systems with each other without relying on a single database, such as a cloud application. 
    - One approach to implement this is EY's Baseline [https://docs.baseline-protocol.org/] Protocol. Baseline works by setting up a workgroup of companies that wish to synchronise records between their databases and build workflows and use the Ethereum blockchain as the state machine reference to determine, which version of a record is the latest. Another approach is Corda's Flows that encode business rules and use a notary chain to record compliant changes in the state of data.
    - Another approach that is novel is Centrifuge 
    - Putting workflows or business rules on a blockchain is one way of using a blockchain to link records across organisations, but does this approach make sense? We suggest that the success of ICOs and DeFi, which are based on freeform and permissionless primitives suggests that rules-bound workflows are not interesting primitives for putting contractual obligations on-chain.
    - The purpose of Send Protocol is to enable tokenisation of agreements allowing them to be traded or securitised. Send Protocol provides a framework for establishing verifiable contractual relationships and payment terms that are manifested as transferrable rights and obligations. The agreements that can be tokenised using Send can be literally any agreement whatsoever, starting with sales agreements, through to financial products, and even shareholder structures.
    - In this article we suggest the appropriate elementary concepts that allow putting wet contracts on-chain into permissionless structures that lend themselves to composability, a key feature of DeFi.
